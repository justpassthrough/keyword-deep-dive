...
| 09-17 | **급등 신호 요일 착시 수정**: 3일 변화율을 '직전 3일'이 아니라 '일주일 전 같은 요일 3일'과 비교. 4주 실측에서 목요일엔 키워드의 47%, 월요일엔 14%가 급등으로 잡히던 문제(주말에 건강 검색이 줄어서) |
| 09-17 | **전문가 갭 → 문서 포화도**: `키워드 약사` 문서수 조회 제거(84%가 '부족' 이상이라 변별력 없음). 블로그 문서수 ÷ 월 검색수 = 포화도(≤3 문서 적음 / ≤30 보통 / 그 위 도배됨). `expert_gap` 필드는 호환용으로 유지, `saturation`·`saturation_label` 추가 |
| 09-17 | **이미 쓴 글 15일 규칙**: 교차검증을 추천 선정 앞으로 옮김(예전엔 TOP7에 쓴 글이 그대로 들어감). 쓴 지 15일 지난 글은 `already_written=false` + `previously_written`/`days_since_written` 로 재작성 후보 |
| 09-17 | **표기 변형 합치기**: 띄어쓰기·어순만 다른 키워드(활성 엽산/활성엽산, 마운자로 일본/일본 마운자로)를 하나로, `variants`에 기록. 뿌리당 개수 제한은 두지 않음(사용자 결정) |
| 09-17 | **쇼핑 검색 API 종료 확인**(404 SE05) → 쇼핑 상품수 브랜드 판별 제거. 브랜드는 이름 목록 + (예정) 실제 검색 화면 확인 단계가 맡음 |
| 09-17 | **급등 키워드 2차 cosearch 제거**: 230개를 10분간 긁어 접힌 섹션 10개만 남기던 단계. `deep_cosearch_trending`은 빈 목록으로 유지 |
| 09-17 | **검색 화면 확인(serp) 연동**: 미니PC(집 IP)의 `video-auto/scripts/serp-check.mts`(작업 스케줄러 `Auto_SerpCheck`, 30분마다·새 스캔 있을 때만·회당 30개)가 상위 후보를 네이버 모바일 검색으로 열어 `data/serp.json`에 기록 → dive.py가 `compounds[].serp`로 붙임. 공식 블로그 검색 API 정렬은 실제 화면과 달라 못 씀 |
| 09-17 | **점수 식 교체(축: 내가 이길 수 있는 키워드)**: `log10(월검색) × 모멘텀 × 자리 × 쇼핑덮임 × 문서 포화도 × 약사보정`. 자리=블로그 영역 위치(<1,000px 1.5 / <2,500 1.25 / <5,000 1.0 / 그 아래 0.7), 쇼핑이 위를 덮으면 ×0.85, 포화도 1.2/1.0/0.8, '약국 ○○'은 ×1.1. 광고 경쟁도는 점수에서 제외(표시만). 화면 미확인 키워드는 같은 뿌리 확인분의 중앙값을 빌림. **추천(TOP7·1순위)은 화면을 확인했고 블로그가 5,000px 안에 나오는 자리만**. 실측: 영양제는 쇼핑 블록이 위를 덮고 블로그가 5,000~10,000px 아래, 처방약은 블로그가 맨 위. 배수는 초안 — 2~3주 뒤 실제 유입으로 재조정 |
| 09-17(밤) | **자리 비중 축소**(사용자 판단: 쇼핑이 위에 있어도 블로그를 찾는 사람은 내려서 본다). 자리 1.5/1.25/1.0/0.7 → 1.1/1.05/1.0/0.95, 쇼핑 덮임 0.85 → 0.97. 추천의 '블로그 5,000px 안' 조건 제거(화면 미확인 글감도 추천 가능). 신제품 브랜드 꼴·지역 검색 제외는 유지 |
